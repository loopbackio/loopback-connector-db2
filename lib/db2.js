...
function DB2(settings) {
  ...
  this.pool = this.client = new Driver.Pool();
  this.connStr =
    'DRIVER={DB2}' +
...
DB2.prototype.dropTable = function(model, cb) {
  var sql =
      ...
      this.schema + '.' + this.tableEscaped(model) + '\';\nEND';
  this.execute(sql, cb);
};
...
DB2.prototype.columnDataType = function(model, property) {
  ...
  if (colType) {
    colType = this.buildColumnType(property);
  }
