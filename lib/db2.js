...
DB2.prototype.update = function(model, where, data, options, cb) {
  debug('DB2.prototype.update (enter): where=%j, data=%j, options=%j\n',
        where, data, options);
  var self = this;
 ...
 * @param {Function} cb The callback function
 */
DB2.prototype.replaceById = function(model, where, data, options, cb) {
  debug('DB2.prototype.replaceById (enter): where=%j, data=%j, options=%j\n',
        where, data, options);
  var self = this;
  var stmt = self.buildReplace(model, where, data, options);
  var id = self.idName(model);
  var sql = 'SELECT \"' + id + '\" FROM FINAL TABLE (' + stmt.sql + ')';
  self.execute(sql, stmt.params, options, function(err, info) {
    if (cb) {
