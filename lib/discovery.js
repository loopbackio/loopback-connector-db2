...
function mixinDiscovery(DB2, db2) {
  ...
  DB2.prototype.buildQuerySchemas = function(options) {
    debug('DB2.prototype.buildQuerySchemas: options=%j', options);
  ...
  DB2.prototype.buildQueryTables = function(options) {
    debug('DB2.prototype.buildQueryTables: options=%j', options);
    let sqlTables = null;
  ...
  DB2.prototype.buildQueryViews = function(options) {
    debug('DB2.prototype.buildQueryViews: options=%j', options);
    let sqlViews = null;
    if (options.views) {
      const schema = options.owner || options.schema;
  ...
  DB2.prototype.buildQueryColumns = function(schema, table) {
    debug('DB2.prototype.buildQueryColumns: schema=%j, table=%j',
      schema, table);
    let sql = null;
    if (schema) {
  ...
  DB2.prototype.buildQueryPrimaryKeys = function(schema, table) {
    debug('DB2.prototype.buildQueryPrimaryKeys: schema=%j, table=%j',
      schema, table);
    let sql = 'SELECT TRIM(tabschema) AS "owner",' +
      ' TRIM(tabname) AS "tableName",' +
      ' TRIM(colname) AS "columnName",' +
      ' colseq AS "keySeq",' +
      ' constname AS "pkName"' +
  ...
  DB2.prototype.buildQueryForeignKeys = function(schema, table) {
    debug('DB2.prototype.buildQueryForeignKeys: schema=%j, table=%j',
      schema, table);
    let sql =
  ...
  DB2.prototype.buildQueryExportedForeignKeys = function(schema, table) {
    debug('DB2.prototype.buildQueryExportedForeignKeys: schema=%j, table=%j',
      schema, table);
    let sql = 'SELECT TRIM(a.constraint_name) AS "fkName",' +
  ...
  DB2.prototype.buildPropertyType = function(columnDefinition) {
    debug('DB2.prototype.buildPropertyType: columnDefinition=%j',
      columnDefinition);
